...
var newData = data;
const table = document.getElementById("user_list_table");
let deleteBtn = document.getElementsByClassName("op_btn_delete");
var arr = [...deleteBtn];
let viewButton = document.getElementsByClassName("op_btn_view");
var viewButtonArr = [...viewButton];
let editButton = document.getElementsByClassName("op_btn_edit");
var editButtonArr = [...editButton];

const cancelSearch = document.getElementById("cancel_search");
...
const showUsers = () => {
    newData.forEach((item) => {
        ...
        _tr.innerHTML = `<td>${item.firstName}</td>
                        <td>${item.lastName}</td>
                        <td>${item.email}</td>
                        <td>${item.point}</td>
                        <td>${item.phone}</td>
                        <td>${item.company}</td>
                        <td> <button class="op_btn_view">View</button> <button class="op_btn_edit">Edit</button> <button class="op_btn_delete">Delete</button></td>
                        `;
        table.children[0].appendChild(_tr);
    });

    //Add delete button to every user data
    //Delete user from "data" array
    deleteBtn = document.getElementsByClassName("op_btn_delete");
    arr = [...deleteBtn];
    arr.forEach((item) => {
        item.addEventListener("click", (e) => {
            item.parentElement.parentElement.remove();
            //Grab email and find it's index and delete it by index
            let removedUser =
                e.target.parentElement.parentElement.children[2].innerText;
            const removedUserIs = data.find((value) => removedUser === value.email);
            newData.splice(data.indexOf(removedUserIs), 1);
        });
    });

    viewButton = document.getElementsByClassName("op_btn_view");
    viewButtonArr = [...viewButton];

    viewButtonArr.forEach((item) => {
        item.addEventListener("click", () => {
            let userEmail = item.parentElement.parentElement.children[2].innerText;
            console.log(item.parentElement.parentElement.children[2]);
            location.href = `view_user_info.html?user=${userEmail}`;
        });
    });

    editButton = document.getElementsByClassName("op_btn_edit");
    editButtonArr = [...editButton];

    editButtonArr.forEach((item) => {
        item.addEventListener("click", () => {
            let userEmail = item.parentElement.parentElement.children[2].innerText;
            console.log(item.parentElement.parentElement.children[2]);
            location.href = `edit_user_info.html?user=${userEmail}`;
        });
    });
};

//calling showUsers() for the data load first time
showUsers();

//Remove the table's all data
...
searchBtn.addEventListener("click", () => {
    let input_data = searchText.value.toUpperCase();
    newData = [];
    ...
    if (input_data.length > 0) {
        ...
        data.forEach((item) => {
            if (
                item.firstName.toUpperCase().indexOf(input_data) > -1 ||
                item.lastName.toUpperCase().indexOf(input_data) > -1
            ) {
                newData.push(item);
            }
        });
